...
pheno <- read.csv("univariatePHENOBLUP/traitBLUPs_stress1.csv")
...
for (i in 4:ncol(pheno)){
  ...
  phenoTrn <- pheno[c(1:292), ]
  genoTrn <- geno_imp[c(1:292), ]
  genoTrgt <- geno_imp[c(293:430),]
  phenoTrgt <- pheno[c(293:430),]
  ...
  paTrnStor[[i]] <- cor(rrGebvTrn, phenoTrn[c(1:292),i])
  phenoTrgtPred[[i]] <- rrGebvTrgt
  
}

#combine the loop lists in to dataframes
Stress1.RRblup.TrgtPredPheno.data <- do.call(cbind,phenoTrgtPred[4:78])
Stress1.RRblup.TrnPredAcc.data <- do.call(cbind,paTrnStor[4:78])

#apply phenotype names to dataframes
colnames(Stress1.RRblup.TrgtPredPheno.data) <- c(paste0(colnames(pheno[,4:78]),"_RRTrgtPred"))
colnames(Stress1.RRblup.TrnPredAcc.data) <- c(paste0(colnames(pheno[,4:78]),"_RRTrnCorr"))
rownames(Stress1.RRblup.TrgtPredPheno.data) <- c(paste0(rownames(genoTrgt)))
Stress1.RRblup.TrnPredAcc.data <- as.data.frame(t(Stress1.RRblup.TrnPredAcc.data))
Stress1.RRblup.TrnPredAcc.data.accuracy <- mean(Stress1.RRblup.TrnPredAcc.data[,1],na.rm=TRUE)



#write.csv(Stress1.RRblup.TrnPredAcc.data,"univariateRRBLUP/traitRR_TrnPopPredAcc_stress1.csv")
#write.csv(Stress1.RRblup.TrgtPredPheno.data,"univariateRRBLUP/traitRR_TrgtPopPredPhen_stress1.csv")
